...
export async function exportBackup(user: DbUser, db: LyreDb): Promise<BackupData> {
	...
	await runBatch(db, (h) => {
		...
		for (const t of backup.tags) {
			if (tagExists.get(t.id)) {
				stmts.push(
					h.update(tags).set({ name: t.name }).where(eq(tags.id, t.id)) as unknown as ReturnType<
						typeof h.insert
					>,
				);
			} else {
			...
			if (jobExists.get(j.id)) {
				stmts.push(
					h
						...
						.where(eq(transcriptionJobs.id, j.id)) as unknown as ReturnType<typeof h.insert>,
				);
			} else {
				stmts.push(
			...
			if (transcriptionExists.get(t.id)) {
				stmts.push(
					h
						...
						.where(eq(transcriptions.id, t.id)) as unknown as ReturnType<typeof h.insert>,
				);
			} else {
				stmts.push(
		...
		const recordingIdsInBackup = new Set(backup.recordingTags.map((rt) => rt.recordingId));
		for (const recId of recordingIdsInBackup) {
			stmts.push(
				h
					.delete(recordingTags)
					.where(eq(recordingTags.recordingId, recId)) as unknown as ReturnType<typeof h.insert>,
			);
		}
			...
			if (tokenExists.get(dt.id)) {
				stmts.push(
					h
						...
						.where(eq(deviceTokens.id, dt.id)) as unknown as ReturnType<typeof h.insert>,
				);
			} else {
				stmts.push(

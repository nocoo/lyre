 ...
 */
export function parseTranscriptionResult(raw: AsrTranscriptionResult): ParsedAsrResult {
...
export function createMockAsrProvider(options: MockAsrProviderOptions = {}): AsrProvider {
	const {
		pollsUntilRunning = 1,
		pollsUntilDone = 3,
	...
	return {
		...
		async fetchResult(_transcriptionUrl: string): Promise<AsrTranscriptionResult> {
...
async function safeFetch(
	...
	try {
		...
		if (statusLine === undefined) {
			throw new Error("curl output empty; no status code line");
		}
...
export function createRealAsrProvider(apiKey: string): AsrProvider {
	...
	return {
		...
		async poll(taskId: string): Promise<AsrPollResponse> {
			const { status, body } = await safeFetch(`${DASHSCOPE_BASE_URL}/tasks/${taskId}`, {
				headers,
			});
		...
		async fetchResult(transcriptionUrl: string): Promise<AsrTranscriptionResult> {
			// transcription_url is a presigned OSS URL — no auth needed
			const { status, body } = await safeFetch(transcriptionUrl);

			if (status < 200 || status >= 300) {
				throw new Error(`Failed to fetch transcription result (${status}): ${body}`);
			}

...
export async function listTagsHandler(ctx: RuntimeContext): Promise<HandlerResponse> {
...
export async function deleteTagHandler(ctx: RuntimeContext, id: string): Promise<HandlerResponse> {
	if (!ctx.user) return unauthorized();
	const { tags } = makeRepos(ctx.db);
	const tag = await tags.findByIdAndUser(id, ctx.user.id);
	if (!tag) return notFound("Tag not found");

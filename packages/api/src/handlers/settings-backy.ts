...
export async function getBackySettingsHandler(ctx: RuntimeContext): Promise<HandlerResponse> {
...
export async function testBackySettingsHandler(ctx: RuntimeContext): Promise<HandlerResponse> {
	if (!ctx.user) return unauthorized();
	const settings = await readBackySettings(ctx.user.id, ctx.db);
...
export async function backyHistoryHandler(ctx: RuntimeContext): Promise<HandlerResponse> {
	if (!ctx.user) return unauthorized();
	const settings = await readBackySettings(ctx.user.id, ctx.db);
	if (!settings.webhookUrl || !settings.apiKey) {
		return badRequest("Webhook URL and API key must be configured first");
	...
	if (!result.ok) {
		return json({ error: result.error ?? `HTTP ${result.status}` }, 502);
	}
	return json(result.data);
}

export async function generatePullKeyHandler(ctx: RuntimeContext): Promise<HandlerResponse> {
	if (!ctx.user) return unauthorized();
...
export async function deletePullKeyHandler(ctx: RuntimeContext): Promise<HandlerResponse> {
	if (!ctx.user) return unauthorized();
...
export async function backyPullHeadHandler(ctx: RuntimeContext): Promise<HandlerResponse> {
	const auth = await authenticateWebhookKey(ctx);
	if (!auth.ok) return auth.res;
	return { kind: "empty", status: 200 };
}

export async function backyPullPostHandler(ctx: RuntimeContext): Promise<HandlerResponse> {
	const start = Date.now();
	const auth = await authenticateWebhookKey(ctx);
	if (!auth.ok) return auth.res;
	...
	if (!pushConfig.webhookUrl || !pushConfig.apiKey) {
		return json(
			{
				ok: false,
				error: "Push configuration not set. Configure Backy webhook URL and API key first.",
			},
			422,
		);
	}

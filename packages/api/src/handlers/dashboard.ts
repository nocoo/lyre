...
export async function dashboardHandler(ctx: RuntimeContext): Promise<HandlerResponse> {
	...
	for (const rec of allRecordings) {
		...
		monthlyDurationMap.set(monthKey, (monthlyDurationMap.get(monthKey) ?? 0) + (rec.duration ?? 0));
	...
	const recordingStats: RecordingStats = {
		...
		byMonth,
		durationByMonth,
		byStatus: (["uploaded", "transcribing", "completed", "failed"] as RecordingStatus[]).map(
	...
	const ossSizeByMonthMap = new Map<string, { uploads: number; results: number }>();

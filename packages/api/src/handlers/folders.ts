...
import { badRequest, type HandlerResponse, json, notFound, unauthorized } from "./http";

export async function listFoldersHandler(ctx: RuntimeContext): Promise<HandlerResponse> {
...
export async function getFolderHandler(ctx: RuntimeContext, id: string): Promise<HandlerResponse> {
	if (!ctx.user) return unauthorized();
	const { folders } = makeRepos(ctx.db);

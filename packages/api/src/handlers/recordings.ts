...
const VALID_STATUSES = ["all", "uploaded", "transcribing", "completed", "failed"] as const;
...
export async function listRecordingsHandler(
	...
	const pageSize = Math.min(100, Math.max(1, parseInt(input.pageSize ?? "10", 10) || 10));
...
async function cleanupResultObjects(jobId: string, ctx: RuntimeContext): Promise<void> {
...
export async function playUrlHandler(ctx: RuntimeContext, id: string): Promise<HandlerResponse> {
	if (!ctx.user) return unauthorized();
...
export async function wordsHandler(ctx: RuntimeContext, id: string): Promise<HandlerResponse> {
	if (!ctx.user) return unauthorized();
...
export async function transcribeRecordingHandler(
	...
	try {
		...
		const rawModel =
			(await repos.settings.findByKey(ctx.user.id, "asr.model"))?.value ?? DEFAULT_ASR_MODEL;

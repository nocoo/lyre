...
describe("get/update/deleteFolderHandler", () => {
	it("404 when folder not found", async () => {
		...
		expect((await updateFolderHandler(ctx, "missing", { name: "x" })).status).toBe(404);
	...
	it("get/update/delete round-trip", async () => {
		...
		expect((await updateFolderHandler(ctx, id, { name: "F1b" })).status).toBe(200);
	...
	it("401 for anonymous", async () => {
		...
		expect((await updateFolderHandler(setupAnonCtx(), "x", {})).status).toBe(401);

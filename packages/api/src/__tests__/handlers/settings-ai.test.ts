...
describe("settings-ai sync handlers", () => {
	...
	it("400 invalid provider", async () => {
		const { ctx } = await setupAuthedCtx();
		expect((await updateAiSettingsHandler(ctx, { provider: "bogus" })).status).toBe(400);
	});
	it("400 invalid sdkType", async () => {
		const { ctx } = await setupAuthedCtx();
		expect((await updateAiSettingsHandler(ctx, { sdkType: "weird" })).status).toBe(400);
	});
	it("400 invalid authType", async () => {
		const { ctx } = await setupAuthedCtx();
		expect((await updateAiSettingsHandler(ctx, { authType: "weird" })).status).toBe(400);
	});
	it("update persists authType and GET returns it", async () => {
		...
		expect(persisted.find((s) => s.key === "ai.authType")?.value).toBe("bearer");

...
import { type DbTag, recordingTags, tags } from "../schema";
...
export function makeTagsRepo(db: LyreDb) {
	const repo = {
		...
		async findByIdAndUser(id: string, userId: string): Promise<DbTag | undefined> {
			return await db
				.select()
				.from(tags)
		...
		async findByNameAndUser(name: string, userId: string): Promise<DbTag | undefined> {
			return await db
				.select()
				.from(tags)
		...
		async create(data: { id: string; userId: string; name: string }): Promise<DbTag> {
			const now = Date.now();
			return await db
				.insert(tags)
		...
		async update(id: string, data: { name: string }): Promise<DbTag | undefined> {
			const rows = await db
				.update(tags)
				.set({ name: data.name })
				.where(eq(tags.id, id))
		...
		async findTagsForRecording(recordingId: string): Promise<DbTag[]> {
			...
			return await db.select().from(tags).where(inArray(tags.id, tagIds)).all();
		},

		async setTagsForRecording(recordingId: string, tagIds: string[]): Promise<void> {
			await runBatch(db, (h) => {
				const stmts = [h.delete(recordingTags).where(eq(recordingTags.recordingId, recordingId))];
		...
		async clearTagsForRecording(recordingId: string): Promise<void> {
			await db.delete(recordingTags).where(eq(recordingTags.recordingId, recordingId)).run();
		},
	};

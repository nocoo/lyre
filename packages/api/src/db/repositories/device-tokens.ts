...
import { type DbDeviceToken, deviceTokens } from "../schema";
...
export function makeDeviceTokensRepo(db: LyreDb) {
	return {
		...
		async findById(id: string): Promise<DbDeviceToken | undefined> {
			return await db.select().from(deviceTokens).where(eq(deviceTokens.id, id)).get();
		},

		async findByUserId(userId: string): Promise<DbDeviceToken[]> {
			return await db.select().from(deviceTokens).where(eq(deviceTokens.userId, userId)).all();
		},
		...
		async deleteByUserId(userId: string): Promise<number> {
			const result = await db.delete(deviceTokens).where(eq(deviceTokens.userId, userId)).run();

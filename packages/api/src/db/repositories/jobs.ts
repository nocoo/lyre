...
export function makeJobsRepo(db: LyreDb) {
	return {
		async findById(id: string): Promise<DbTranscriptionJob | undefined> {
			return await db.select().from(transcriptionJobs).where(eq(transcriptionJobs.id, id)).get();
		},

		async findByTaskId(taskId: string): Promise<DbTranscriptionJob | undefined> {
			return await db
				.select()
		...
		async findLatestByRecordingId(recordingId: string): Promise<DbTranscriptionJob | undefined> {
			return await db
				.select()
				.from(transcriptionJobs)
		...
		async findByRecordingId(recordingId: string): Promise<DbTranscriptionJob[]> {
			return await db
				.select()
				.from(transcriptionJobs)
				.where(eq(transcriptionJobs.recordingId, recordingId))
		...
		async delete(id: string): Promise<boolean> {
			const result = await db.delete(transcriptionJobs).where(eq(transcriptionJobs.id, id)).run();

...
import { type DbTranscription, transcriptions } from "../schema";
...
export function makeTranscriptionsRepo(db: LyreDb) {
	return {
		async findById(id: string): Promise<DbTranscription | undefined> {
			return await db.select().from(transcriptions).where(eq(transcriptions.id, id)).get();
		},

		async findByRecordingId(recordingId: string): Promise<DbTranscription | undefined> {
			return await db
				.select()
				.from(transcriptions)
		...
		async update(
			...
			if (data.sentences !== undefined) updateData.sentences = JSON.stringify(data.sentences);
		...
		async delete(id: string): Promise<boolean> {
			const result = await db.delete(transcriptions).where(eq(transcriptions.id, id)).run();

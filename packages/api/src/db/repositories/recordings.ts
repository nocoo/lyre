...
export function makeRecordingsRepo(db: LyreDb) {
	const repo = {
		...
		async findById(id: string): Promise<DbRecording | undefined> {
			return await db.select().from(recordings).where(eq(recordings.id, id)).get();
		},
		...
		async update(
			...
			if (data.aiSummaryStatus !== undefined) updateData.aiSummaryStatus = data.aiSummaryStatus;
			if (data.aiSummaryError !== undefined) updateData.aiSummaryError = data.aiSummaryError;
		...
		async delete(id: string): Promise<boolean> {
			const result = await db.delete(recordings).where(eq(recordings.id, id)).run();
		...
		async deleteCascade(id: string): Promise<boolean> {
			...
			await runBatch(db, (h) => [
				...
				h.delete(transcriptionJobs).where(eq(transcriptionJobs.recordingId, id)),

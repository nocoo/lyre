...
import { get, json, post } from "./helpers";

const FIXTURE_PATH = resolve(import.meta.dirname, "..", "fixtures", "dual-track-asr.m4a");
...
describe("DashScope multi-track ASR compatibility probe (Phase 0B)", () => {
	test("dual-track fixture surfaces both system and mic keywords", async () => {
		...
		const hits = (keywords: string[]) => keywords.filter((k) => fullText.includes(k)).length;

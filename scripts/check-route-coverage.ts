...
function discoverSubRoutes(prefixes: Map<string, string>): Route[] {
	...
	for (const file of allFiles) {
		...
		const routeRe = /\b(\w+)\.(get|post|put|delete|patch|head)\(\s*["']([^"']*)["']/g;
		...
		for (const m of src.matchAll(allRe)) {
			...
			const methodChecks = [
				...src.matchAll(/method\s*===?\s*["'](\w+)["']|c\.req\.method\s*===?\s*["'](\w+)["']/g),
			];
...
function main(): void {
	...
	if (uncovered.length === 0) {
		console.log(`All ${uniqueDeclared.length} routes have at least one E2E request.\n`);
		return;
	}

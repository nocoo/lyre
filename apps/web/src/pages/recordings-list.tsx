...
function RecordingsPageInner() {
	...
	const selectedItems = listVM.cards
							...
							<div className="flex flex-wrap gap-1.5" role="group" aria-label="Sort options">
								{SORT_OPTIONS.map((opt) => (
									...
									>
										...
										{sortField === opt.value && (sortDirection === "asc" ? " ↑" : " ↓")}
									</Badge>
								))}
							</div>
						</div>
			...
			<AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							This action cannot be undone. The following recording
							{selectedIds.size !== 1 ? "s" : ""} and all associated transcriptions will be
							permanently deleted:
						</AlertDialogDescription>
					</AlertDialogHeader>

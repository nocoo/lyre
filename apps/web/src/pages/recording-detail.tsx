...
export default function RecordingDetailPage(_props: PageParams) {
	...
	useSetBreadcrumbs([{ label: "Recordings", href: "/recordings" }, { label: "Detail" }]);
...
function RecordingDetailContent({ id }: { id: string }) {
	...
	const [aiSummaryStatus, setAiSummaryStatus] = useState<"running" | "succeeded" | "failed" | null>(
		null,
	);
	const [aiSummaryError, setAiSummaryError] = useState<string | null>(null);
	const [manualStreamingText, setManualStreamingText] = useState<string | null>(null);
	...
	const [saveIndicator, setSaveIndicator] = useState<"saving" | "saved" | "failed" | null>(null);
	...
	const loadDetail = useCallback(async () => {
		try {
			...
			if (res.ok) {
				...
				setNotes(data.notes ?? "");
				setRecordedAtDate(data.recordedAt ? toDateInputValue(data.recordedAt) : "");
	...
	const loadTagsAndFolders = useCallback(async () => {
		const [tagsRes, foldersRes] = await Promise.all([fetch("/api/tags"), fetch("/api/folders")]);
		if (tagsRes.ok) {
	...
	useEffect(() => {
		async function load() {
			setLoading(true);
			const [data] = await Promise.all([loadDetail(), loadTagsAndFolders(), loadAiSettings()]);
			setLoading(false);
	...
	useEffect(() => {
		...
		const pollOnce = async () => {
			...
			if (attempts >= MAX_POLL_ATTEMPTS) {
				...
				setAiSummaryError("Summary generation did not complete in time. Try again.");
				setSaveIndicator("failed");
				stopPolling();
			}
		};
	...
	const handleSaveProperties = useCallback(async () => {
		...
		const origDate = detail?.recordedAt ? toDateInputValue(detail.recordedAt) : "";
		if (recordedAtDate !== origDate) {
			updates.recordedAt = recordedAtDate ? new Date(recordedAtDate).getTime() : null;
		}
	...
	// ── RecordedAt change ──
	const handleRecordedAtChange = useCallback((dateStr: string) => {
	...
	const propertiesDirty =
		...
		recordedAtDate !== (detail.recordedAt ? toDateInputValue(detail.recordedAt) : "");
	...
	return (
		<div className="space-y-5">
							...
							<AlertDialogContent>
								<AlertDialogHeader>
									...
									<AlertDialogDescription>
										...
										&rdquo; and its audio file from storage. This action cannot be undone.
									</AlertDialogDescription>
								</AlertDialogHeader>
								<AlertDialogFooter>
			...
			{(activeJobId || vm.isTranscribing) && <TranscribingCard status={pollStatus} />}
			{vm.job?.isFailed && !activeJobId && <JobErrorCard message={vm.job.errorMessage} />}
				...
				<Popover open={folderOpen} onOpenChange={onFolderOpenChange}>
					...
					<PopoverContent className="w-48 p-0" align="start">
						<Command>
							<CommandList>
								<CommandGroup>
									<CommandItem onSelect={() => onFolderChange(null)} className="gap-2">
										<Check
			...
			<div className="space-y-1.5">
				...
				<div className="flex flex-wrap items-center gap-1.5">
					{selectedTagIds.map((tagId) => {
						...
						const color = getTagColor(tag.name);
						return (
							// biome-ignore lint/a11y/useSemanticElements: tag chip must remain inline for flow layout; keyboard support is provided
							<span
								key={tag.id}
								className={cn(
										...
										{allTags.map((tag) => {
											...
											return (
												...
												>
													...
													/>
													<span className={cn("h-2.5 w-2.5 rounded-full shrink-0", color.bg)} />
													{tag.name}
												</CommandItem>
											);
										})}

					...
					{user.uploads.folders.length > 0 && (
						<div>
							...
							{user.uploads.folders.map((folder) => {
								...
								const allSelected = folderFileKeys.every((k) => selectedKeys.has(k));
								return (
									<FolderRow
										key={folder.id}
										folder={folder}
...
export function OssStorageSection() {
	...
	const handleCleanup = async () => {
		...
		try {
			...
			toast.success(`Deleted ${result.deleted} files (${result.skipped} skipped as non-orphan)`);
	...
	const selectedSize = (() => {
		...
		let size = 0;
		const allFolders = [
			...data.users.flatMap((u) => [...u.uploads.folders, ...u.results.folders]),
	...
	return (
		<div className="space-y-6">
			...
			<div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
				...
				/>
				<SummaryCard label="Users" value={data.users.length.toString()} sub="across all buckets" />
				<SummaryCard
					label="Orphan Files"
			...
			<div className="flex items-center justify-between gap-3">
				<div className="flex items-center gap-2">
					...
					{hasOrphans && (
						<Fragment>
							<Button variant="outline" size="sm" onClick={selectAllOrphans} className="gap-1.5">
			...
			<AlertDialog open={showConfirm} onOpenChange={setShowConfirm}>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							This will permanently delete {selectedKeys.size} files ({formatBytes(selectedSize)})
							from OSS. Only files confirmed as orphans (no matching database record) will be

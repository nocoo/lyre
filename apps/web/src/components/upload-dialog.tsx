		...
		try {
			...
			await new Promise<void>((resolve, reject) => {
				...
				xhr.addEventListener("error", () => {
					...
					reject(
						new Error(
							"Network error during upload. This may be a CORS issue with the storage provider.",
						),
					);
				});
		...
		<Dialog open={open} onOpenChange={handleOpenChange}>
			<DialogContent className="sm:max-w-md">
				<DialogHeader>
					...
					<DialogDescription>
						Upload an audio file for transcription. Supports MP3, WAV, OGG, FLAC, AAC, M4A, and
						WebM.
					</DialogDescription>
				</DialogHeader>
				...
				<DialogFooter>
					...
					<Button
						...
						disabled={
							!file || !title || state === "uploading" || state === "creating" || state === "done"
						}
					>
						{state === "uploading" || state === "creating" ? (
							<>

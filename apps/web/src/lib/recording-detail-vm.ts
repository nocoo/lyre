...
export function toRecordingMetadataVM(detail: RecordingDetail): RecordingMetadataVM {
	return {
		id: detail.id,
		title: detail.title,
...
export function toTranscriptionVM(detail: RecordingDetail): TranscriptionVM | null {
 ...
 */
export function computeEstimatedCost(usageSeconds: number | null): string {
 ...
 */
export function findActiveWordIndex(words: WordVM[], currentTimeSeconds: number): number {
	const currentMs = currentTimeSeconds * 1000;
...
export function toRecordingDetailVM(detail: RecordingDetail): RecordingDetailVM {

...
recordings.post("/batch-delete", async (c) => {
	const body = await c.req.json().catch(() => ({}));
	return toResponse(c, await batchDeleteRecordingsHandler(c.get("runtime"), body));
});
...
recordings.get("/:id", async (c) =>
	toResponse(c, await getRecordingHandler(c.get("runtime"), c.req.param("id"))),
);

recordings.put("/:id", async (c) => {
	const body = await c.req.json().catch(() => ({}));
	return toResponse(c, await updateRecordingHandler(c.get("runtime"), c.req.param("id"), body));
});

recordings.delete("/:id", async (c) =>
	toResponse(c, await deleteRecordingHandler(c.get("runtime"), c.req.param("id"))),
);
...
recordings.post("/:id/summarize", async (c) => {
	...
	if (!transcription?.fullText) {
		return c.json({ error: "No transcription available for this recording" }, 400);
	}
	...
	if (contentType.includes("application/json")) {
		...
		if (raw !== undefined && raw !== null) {
			...
			if (raw.length > MAX_FEEDBACK_LEN) {
				return c.json({ error: `Feedback too long (max ${MAX_FEEDBACK_LEN} chars)` }, 400);
			}
			feedback = raw;
		}
	}
	...
	if (!provider || !apiKey) {
		return c.json({ error: "AI is not configured. Please set up AI in Settings." }, 400);
	}

	// Config parsing errors are user-facing 400s — no status update yet
	// because we haven't started a run.
	let client: ReturnType<typeof createAiModel>;
	try {
